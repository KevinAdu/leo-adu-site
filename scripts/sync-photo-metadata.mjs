    ...
    if (arg === "--only" || arg === "--file") {
      ...
      if (value) {
        value.split(",").forEach((item) => {
          if (item.trim()) onlyFiles.add(item.trim());
        });
      }
      i += 1;
      continue;
...
async function main() {
  ...
  for (const name of enFiles) {
    ...
    if (onlyFiles.size > 0 && !onlyFiles.has(name)) {
      continue;
    }
    ...
    try {
      ...
      const title = typeof en.data.title === "string" ? en.data.title.trim() : "";
      const caption =
        typeof en.data.caption === "string" ? en.data.caption.trim() : "";
      ...
      const jaTitle = ja.data.title;
      const jaCaption = ja.data.caption;

      if (!jaTitle && !jaCaption) {
        skipped.push({ name, reason: "missing-ja-title-caption" });
        continue;
      }

      if (!hasTitle && jaTitle) {
        en.data.title = jaTitle;
      }

      if (!hasCaption && jaCaption) {
        const needsTranslation = hasJapanese(jaCaption);
        en.data.caption = needsTranslation
          ? await translateToEnglish(jaCaption)
          : jaCaption;
      }
